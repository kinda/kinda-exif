...
function ExifImage (options, callback) {
  ...
  if (!options.image) {
    ...
    this.loadAndProcessImageSync(options.image);
  }
}
...
ExifImage.prototype.loadAndProcessImageSync = function (image) {
  if (typeof image === 'string') image = fs.readFileSync(image);
  return this.processImageSync(image);
};
...
ExifImage.prototype.processImageSync = function (data) {
  var offset = 0;
